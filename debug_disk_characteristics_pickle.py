...
save_dir ="/lustre/astro/vitot/analysis"
debug_verbose = True
sink_tag = 164
secondary_sink_tag = 165
...
def shape_velocities(yt_shape,sink_position,sink_velocity,spin_axis,verbose = debug_verbose):
    ...
    s = spin_axis
    #s = l/yt.YTQuantity(np.linalg.norm(l),str(l.units)) ##### here
    ...
    angular_vectors = yt.YTArray([ yt.ucross(s,e_r[:,k]) for k in range(time_steps)]).T
    ...
    radial_vectors = yt.YTArray([ yt.ucross(s,angular_vectors[:,k]) for k in range(time_steps)]).T
    radial_velocities = yt.YTArray([dv[:,k].dot(radial_vectors[:,k]) for k in range(time_steps)]) 
    z_velocites = yt.YTArray([dv[:,k].dot(s) for k in range(time_steps)])  #z direction is defined by s
...
if os.path.exists(pickle_name) == True or len(s['x']) <= sink_tag :
    ...
    save_units = {'rs': 'au','disk_size':'au', 'v_phi_shellaverage': str(v_phi_shellaverage.units) ,
    ...
    if len(s['x'])> max(sink_tag,secondary_sink_tag) :  # if it is a binary system
        ...
        dm2 = s['dm'][sink_tag]*units['mass_unit']
        dt2 = (s['snapshot_time'] - s['tflush'])*units['time_unit']
        accretion_rate_secondary = dm2/dt2
        luminosity_secondary = (0.5*g*s['m'][sink_tag]*units['mass_unit']*accretion_rate_primary/(4*R_sun)).in_units('Lsun')
